...
*/
struct v4l2_device {
	struct device *dev; //指向设备模型的指针
#if defined(CONFIG_MEDIA_CONTROLLER)
	struct media_device *mdev; //指向一个媒体控制器的指针
#endif
	struct list_head subdevs; //管理子设备的双向链表，所有注册到的子设备都需要加入到这个链表当中
	spinlock_t lock;//全局锁
	char name[V4L2_DEVICE_NAME_SIZE];//设备名称
	void (*notify)(struct v4l2_subdev *sd,//通知回调函数，通常用于子设备传递事件，这些事件可以是自定义事件
			unsigned int notification, void *arg);
	struct v4l2_ctrl_handler *ctrl_handler;//控制句柄
	struct v4l2_prio_state prio;//设备的优先级状态，一般有后台，交互，记录三种优先级，依次变高
	struct kref ref;//本结构体的引用追踪
