...
static int i2c_device_probe(struct device *dev)
{
	...
	else if (driver->probe) // 总线设备驱动的探测初始化
...
/*
通过device的device_node查找相应的adapter结构，
...
/*
当一个adapter 加人系统后，说明增加了一个I2C总线连接，而通常I2C总线是在硬件板上直接进行连接，
这就需要进行总线的遍历以及总线设备与相应驱动的绑定
...
/*
i2c_add_adapter和i2c_add_numbered_adapter是I2C adapter的注册接口，
它们的区别是：i2c_add_adapter会自动分配adapter ID（adapter->nr），
i2c_add_numbered_adapter则可以指定ID（adapter->nr需要时有效值，否则会调用i2c_add_adapter自动分配）。
*/
/**
...
/*
通过ID获得指定adapter的指针，由于该接口会尝试调用try_module_get增加模块的引用计数，
因此使用完毕后，需要调用i2c_put_adapter将引用计数减去
*/
