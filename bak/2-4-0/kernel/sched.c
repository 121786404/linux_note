...
handle_softirq_back:
	 ...
	 */
	/*
	 * 取得下一个将要被装入CPU的进程的struct_task。
	 * typedef struct{task_struct *curr; cycles_t last_schedule;}sched_data;
	 * */
	...
	/*如果调度策略为SCHED_RR,则移动到RR队列尾*/
	if (prev->policy == SCHED_RR)
		goto move_rr_last;
move_rr_back:

	/*检查进程的意愿状态，即想要变为哪种状态。
	 * 如果进程将要以TASK_INTURRUPTIBLE状态放弃
	 * CPU的使用权，则先检查是否有信号需要处理，
	 * 如果有信号需要处理，则转而处理信号然后
	 * 直接退出*/
	switch (prev->state) {
...
still_running_back:
	...
	prepare_to_switch();
	{
		...
		/*如果将要被投入运行的进程是一个内核进程，则没有用户空间
		 * 也就是说mm = NULL,这时候就向前一个进程借用一个active_mm,
		 * 这个active_mm是内核空间映射。因为每个进程的内核空间映射都是
		 * 一致的，所以可用之*/
		if (!mm) {
			...
			/*如果进程拥有自己的mm结构，那必然等于active_mm结构*/
			if (next->active_mm != mm) BUG();
			/*进行进程空间映射的切换。
			 *进程切换时，分两部分，一部分是地址映射的切换，
			 * 另一部分是运行现场的切换。地址的切换仅仅是把
			 * mm中的映射表物理地址装入CR3。
			 * */
...
move_rr_last:
	if (!prev->counter) {
		/*counter是RR调度策略的时间片，在每个systick中断中都会递减
		 * 一旦时间片用完，就将时间片按照优先级换算重新设置时间片，
		 * 然后移动到RR队列的队尾，每次调度的时候，队列前面的相同
		 * 优先级进程具有优先权。也就是说，在调度时，先遍历RR运行
		 * 队列，找到优先级最高的进程投入运行，但对于相同优先级
		 * 的进程，排在队列前头的进程具有优先权，将会得到运行权*/
