 ...
 */
/*
i2c_driver 主要负责总线相关以及电源管理相关的操作，操作接口中并不涉及实际的信息传输，
可以说该驱动主要是处理总线和设备模型相关的事务，并不涉及功能的事务。
功能型的事务涉及操作的具体内容，是在功能型驱动中通过I2C 的接口实现的。
而功能型的事务需要有总线设备的信息，这就要求功能型驱动管理实体中能够包含i2c_client 的信息。
这个桥梁是与I2C总线事务相关的，所以该工作自然就由i2cdriver 来完成，具体的接口就是probe ，
这样整体上各种实体就关联起来了。
*/
struct i2c_driver {
	unsigned int class;
 ...
 */
/*
i2c_algorithm中定义了操作接口，通过这些操作接口可以实现标准的i2c 总线操作。
由于不同设备的操作方法不同，该接口实际是实现具体设备总线控制器的总线操作逻辑。

有两个操作接口，但是两个并不都需要提供，通常只要提供标准i2c 接口master_xfer 即可。
通过标准的i2c 操作可以模拟smhus 操作， i2c总线框架提供函数i2c_smbus_xfer_emulated 来模拟相应的操作。
在一次传输中可以批量做多次处理，每个处理都是一次i2c_msg 的交互操作，
具体i2c_msg的个数由参数num 指定
*/
struct i2c_algorithm {
	...
	/*
       I2C协议有关的数据传输接口，输入参数是struct i2c_msg类型的数组（大小由num指定）。
       返回值是成功传输的msg的个数，如有错误返回负值
	*/
...
/*
驱动调用来检查I2C adapter支持的功能，包括（具体可参考include/uapi/linux/i2c.h中的定义和注释）：
*/
 ...
 */
 /*
 struct i2c_adapter是用于标识物理的I2C总线（physical i2c bus），
 且该总线需要有一套用于访问slave设备的算法(通过I2C总线发送和接收数据的方法)， 它保存在algo指针中
 */
struct i2c_adapter {
	struct module *owner;
/*
class，该I2C bus支持哪些类型的slave device，只有匹配的slave device才能和bus绑定。
参考include/linux/i2c.h中的定义和注释 
*/	
...
/*
    由于总线链接多个设备，该锁保护单一总线操作的原子性
*/
	struct rt_mutex bus_lock;
	struct rt_mutex mux_lock;

/*
基于I2C传输的特性，不一定每一次总线访问（发送或者接收数据）都会成功，在传输失败的时候，可以选择重试。
重试的逻辑由I2C core自行完成，但I2C controller driver需要设定重试的次数，这就是retries字段的意义。
另外，有些consumer对结果的返回是有时间要求的，因此不能无节制的重试，timeout字段（单位为jiffies）在retries基础上，
增加了时间限制，超过这个时间，就不能重试了
*/
...
可由I2C controller driver在注册adapter时指定，或者通过DTS解析（后面会介绍），或者自动分配。
*/
	int nr;
	char name[48];
...
/* DDC是数字显示通道（Digital Display Channel）的意思， 通常用于显示设备信息的获取 */
...
/*
获取指定adapter所支持的功能，i2c_check_functionality可用于检查指定adapter是否具备指定功能
*/
