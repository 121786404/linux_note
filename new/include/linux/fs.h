...
struct file {
	...
	atomic_long_t		f_count; /* 用于记录对struct file对象的使用计数
					  * 当close文件时只有f_count为0时才真正
					  * 执行关闭操作*/
	...
	*/
	...
	struct address_space	*f_mapping;	/*指向一个addres_space结构，这个结构封装了文件的读写缓存页面*/
...
struct super_block {
	...
	unsigned long		s_magic;	/*是魔术数字，每个文件系统都有一个魔术数字*/
	struct dentry		*s_root;	/*是指向文件系统根的指针*/
