...
struct usb_device_descriptor {
	...
	__le16 bcdUSB; //USB设备所遵循的协议版本号，如2.0协议为0x0200。
	/*
	USB设备类代码，由USB-IF分配，
	如果是0x00，表示由接口描述符来指定,	   有可能该USB设备是一个复合设备，USB设备的各个接口相互独立，分别属于不同的设备类）。
	如果是0x01~0xfe，表示为USB-IF定义的设备类，例如0x03为HID设备，0x09为HUB设备。
	如果是0xff，表示由厂商自定义设备类型。
	*/
	__u8  bDeviceClass;
	/*
	USB子类代码，由USB-IF分配，
	如果bDeviceClass为0x00，那么该字段也必须为 0x00，
	其它情况可以参考USB关于对于USB Device Class的定义。
	*/
	__u8  bDeviceSubClass;
	/*
	协议代码，由USB-IF分配，如果bDeviceClass和bDeviceSubClass定义为0x00，那么该  字段也必须为0x00。
	1 :Video Control
	2 :Video Streaming
	*/
	__u8  bDeviceProtocol;
	__u8  bMaxPacketSize0; //端点0最大数据包长度，必须为8、16、32和64.
	__le16 idVendor; //厂商ID，由USB-IF分配，需要向USB-IF组织申请。
	__le16 idProduct; //产品ID，由厂商指定。
	__le16 bcdDevice; //设备序列号，由厂商自行设置
	__u8  iManufacturer; //用于描述厂商的字符串描述符索引
	__u8  iProduct; //用于描述产品的字符串描述符索引
	/*
	用于描述产品序列号的字符串描述符索引，注意，所有的字符串描述符是可选的，
	如果没有字符串描述符，指定这些索引为0x00。
	*/
	__u8  iSerialNumber;
	//配置描述符数量
