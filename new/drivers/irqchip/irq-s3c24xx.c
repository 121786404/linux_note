...
static struct irq_chip s3c_irq_chip = {
    .name= "s3c",
    .irq_ack= s3c_irq_ack,  // 清除中断
    .irq_mask= s3c_irq_mask, // 屏蔽中断
    .irq_unmask= s3c_irq_unmask, // 打开中断
    .irq_set_type= s3c_irq_type, //根据不同的触发类型设置不同的处理函数
...
static void s3c_irq_demux(struct irq_desc *desc) // 这里的desc是主中断的virq的irq_desc
{
    struct irq_chip *chip = irq_desc_get_chip(desc); //获得主中断的irq_chip
    struct s3c_irq_data *irq_data = irq_desc_get_chip_data(desc);
    struct s3c_irq_intc *intc = irq_data->intc; // 参考s3c24xx_irq_xlate_of函数
    struct s3c_irq_intc *sub_intc = irq_data->sub_intc;  // 参考s3c24xx_irq_xlate_of函数
...
static int s3c24xx_irq_map_of(struct irq_domain *h, unsigned int virq,
                            irq_hw_number_t hw)
{
    unsigned int ctrl_num = hw / 32; //判断该hwirq属于主还是子中断控制器
    unsigned int intc_hw = hw % 32; // 得到跟寄存器对应的硬件中断号
    struct s3c_irq_intc *intc = s3c_intc[ctrl_num]; // 获得中断控制器对应的结构体
    struct s3c_irq_intc *parent_intc = intc->parent;
    struct s3c_irq_data *irq_data = &intc->irqs[intc_hw]; // 每个中断都会有一个s3c_irq_data结构体

    /* attach controller pointer to irq_data */
    irq_data->intc = intc;
    irq_data->offset = intc_hw; //跟硬件寄存器对应的硬件中断号，也就是对应的位号

    // 将virq对应的irq_desc的handle_irq初始化为handle_edge_irq，
    // 下面在s3c_irq_type中会根据中断触发类型再次修改
    if (!parent_intc) // 主中断控制器
            ...
            irq_hw_number_t *out_hwirq, unsigned int *out_type)
{
    ...
    if (WARN_ON(intsize < 4))  //如果参数个数不能小于4
        return -EINVAL;

    // 从这里知道，第一个参数不能大于2，只能是0和1， 0表示主中断，2表示子中断
    if (intspec[0] > 2 || !s3c_intc[intspec[0]]) {
        pr_err("controller number %d invalid\n", intspec[0]);
        return -EINVAL;
    }
    
    // s3c_intc[0]表示主中断控制器，s3c_intc[1]表示子中断控制器
    intc = s3c_intc[intspec[0]];

    // 第三个参数表示的是硬件中断号，从这里知道，主中断的硬件中断是0~31，子中断的硬件中断是32及其以上
    *out_hwirq = intspec[0] * 32 + intspec[2];

    // 第四个参数表示的是中断类型
    *out_type = intspec[3] & IRQ_TYPE_SENSE_MASK;

    // 如果是主中断，则intc->parent为NULL， 否则非空
    parent_intc = intc->parent;
    if (parent_intc) {  // 子中断
        irq_data = &intc->irqs[intspec[2]];
        // 对于子中断，第二个参数才有意义，表示该子中断所隶属的主中断的硬件中断号
        irq_data->parent_irq = intspec[1];
        parent_irq_data = &parent_intc->irqs[irq_data->parent_irq];
        parent_irq_data->sub_intc = intc;
        // sub_bits中记录该主中断下的子中断被被使用的情况
        ...
        // 这里设置irqno对应的irq_desc的handle_irq为s3c_irq_demux
            ...
            struct s3c24xx_irq_of_ctrl *s3c_ctrl, int num_ctrl)
{
    ...
    // 对中断控制器的reg属性所表示地址空间进行映射
    reg_base = of_iomap(np, 0);
	if (!reg_base) {
		pr_err("irq-s3c24xx: could not map irq registers\n");
		return -EINVAL;
	}
    // 为该中断控制器创建irq_domain，一共支持64个中断，对应的irq_domain_ops是s3c24xx_irq_ops_of
    ...
    for (i = 0; i < num_ctrl; i++) {
        ...
        // 主和子中断控制器各分配一个s3c_irq_intc
        intc = kzalloc(sizeof(struct s3c_irq_intc), GFP_KERNEL);
		if (!intc)
			return -ENOMEM;
        // 主和子中断控制器共享一个irq_domain
        intc->domain = domain;
        // 为主和子中断控制器下的每个中断各分配一个s3c_irq_data结构体
        ...
        if (ctrl->parent) {  // 子中断控制器
            intc->reg_pending = reg_base + ctrl->offset; // SUBSRCPND
            intc->reg_mask = reg_base + ctrl->offset + 0x4; // INTSUBMSK
                        
            // 由于先处理的是s3c2410_ctrl[0]，所以在处理子中断的时候，*(ctrl->parent)非空
            // 是主中断控制器对应的s3c_irq_intc
            if (*(ctrl->parent)) {
            ...
            intc->reg_pending = reg_base + ctrl->offset;  //SRCPND
            intc->reg_mask = reg_base + ctrl->offset + 0x08; //INTMSK
            intc->reg_intpnd = reg_base + ctrl->offset + 0x10; //INTPND
        }

        s3c24xx_clear_intc(intc);  // 清除中断
        s3c_intc[i] = intc;
    }
    // 将handle_arch_irq设置为s3c24xx_handle_irq， 这样在发生中断后，会从汇编entry-armv.S中
    // 调转到s3c24xx_handle_irq
    set_handle_irq(s3c24xx_handle_irq);
