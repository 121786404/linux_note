...
#define DMAC_MODE_NS	(1 << 0)
	...
	unsigned int	data_bus_width:10;  /* In number of bits */ //AXI接口数据宽度(64bit)
	unsigned int	data_buf_dep:11;//MFIFO的深度,值为127+1
...
struct pl330_reqcfg {
	...
	unsigned brst_size:3; /* in power of 2 */ //2的幂次方，用于设置ccr中dst_burst_size和src_burst_size
...
struct pl330_dmac {
	...
	struct dma_pl330_chan *peripherals;  /* keep at end */ //有8个dma_pl330_chan结构体
	int quirks;
};
					 ...
					 spinlock_t *lock)
{
	...
	if (!list_empty(pool)) {//检测desc_pool是否为空
        //获取第一个desc
...
static inline int get_burst_len(struct dma_pl330_desc *desc, size_t len)
{
	...
	burst_len *= pl330->pcfg.data_buf_dep / pl330->pcfg.num_chan; //执行这一步后，burst_len为MFIFO的大小（1024B）
