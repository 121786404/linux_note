 ...
 */
 /*
early console和u-boot使用相同的串口，并且使用相同的配置，因此不需要额外的初始化操作。

这句话意思是不是说只要uboot阶段能成功驱动串口打印出信息，Linux内核初始化早期阶段如果想做early_serial_console功能的话，
就不用再初始化配置串口了，直接操作串口发送数据就行了。因为这个时候，串口波特率设置，控制寄存器设置都是沿用uboot阶段的配置。
换句话说，就是uboot阶段对SOC各个外设控制器（SPI,I2C,UART,lcd,sdio...还有它们的clock tree设置），
在Linux kernel初始化阶段不会自动清零，除非手动再次执行外设相关驱动（比如在do_basic_setup里面执行各个外设驱动init函数），再次去配置寄存器。
*/
...
static int pl011_probe(struct amba_device *dev, const struct amba_id *id)
{
	...
	// dev->irq[0] 来自 of_amba_device_create->irq_of_parse_and_map
