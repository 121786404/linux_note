 ...
 */
/*
定义了NAND 驱动中对NAND 芯片最基本的操作函数和操作流程，如擦除、读写page 、读写oob 等。
当然这些函数都只是进行一些default 的操作，若你的系统在对NAND 操作时有一些特殊的动作，
则需要在你自己的驱动代码中进行定义，然后Replace 这些default 的函数。
*/
...
read_retry:
			if (nand_standard_page_accessors(&chip->ecc))
				chip->cmdfunc(mtd, NAND_CMD_READ0, 0x00, page); // 发送对应的读页命令
...
int nand_scan_tail(struct mtd_info *mtd)
{
    ...
    /*
       ...
       Nand_scan是在初始化nand的时候对nand进行的一步非常重要的操作， 
