...
repeat:
	...
	*/
	if (fd < files->next_fd)
		fd = files->next_fd;
    /*
        当小于当前文件表支持的最大文件描述符个数时, 
        利用位图找到未用的文件描述符。
        如果大于max_fds怎么办呢?如果大于当前支持的最大文件描述符, 
        那它肯定是未用的, 就不需要用位图来确认了。
      */
	if (fd < fdt->max_fds)
    ...
    /* 
      只有在start小于next_fd时, 才需要更新next_fd, 
      以尽量保证文件描述符的连续性。*/
...
int __close_fd(struct files_struct *files, unsigned fd)
{
	...
	file = fdt->fd[fd];
	/*
      检查file是否为NULL。正常情况下, file一定不为NULL。
      */
	if (!file)
		goto out_unlock;
	/* 将对应的filp置为0*/
...
__releases(&files->file_lock)
{
    ...
    /*  
        ...
        那么为什么不对newfd进行检查呢？因为检查比设置位图更消耗CPU  
        */ 
	__set_open_fd(fd, fdt);
    /*  
