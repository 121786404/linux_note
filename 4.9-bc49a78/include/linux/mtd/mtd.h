...
/*
每个分区也被实现为一个mtd_info，如果有两个MTD原始设备，每个上有三个分区，在系统中就一共有6个mtd_info结构，
这些mtd_info的指针被存放在名为mtd_table的数组里.
*/
struct mtd_info {
	u_char type; //MTD 设备类型  MTD_RAM、MTD_ROM、MTD_NORFLASH、MTD_NANDFLASH
	uint32_t flags;//MTD设备属性标志         MTD_WRITEABLE、MTD_BIT_WRITEABLE、MTD_NO_ERASE、MTD_POWERUP_LOCK
	...
	const struct mtd_ooblayout_ops *ooblayout;//nand ecc布局 
...
/*
MTD通知器，加入/删除MTD设备和原始设备时调用的函数，在设备层，当MTD字符设备或块设备注册时，
如果定义了CONFIG_DEVFS_FS，则会将一个mtd_notifier加入MTD原始设备层的mtd_notifiers链表，
其中的函数会在两种情况下被调用,一是加入/删除新的MTD字符/块设备时，
此时调用该MTD字符/块设备的notifier对下层所有的MTD原始设备操作一遍，
二是加入/删除新的MTD原始设备时，此时调用所有的notifier对该原始设备执行一遍。
*/
struct mtd_notifier {
