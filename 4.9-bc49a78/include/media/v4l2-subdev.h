...
struct v4l2_subdev {
	...
	struct media_entity entity;//媒体控制器的实体，和v4l2_device
#endif
	struct list_head list;
	struct module *owner;
	bool owner_v4l2_dev;
	u32 flags;
	struct v4l2_device *v4l2_dev;//指向一个v4l2设备
	const struct v4l2_subdev_ops *ops;//子设备的操作函数集
	const struct v4l2_subdev_internal_ops *internal_ops;//子设备的内部操作函数集
	struct v4l2_ctrl_handler *ctrl_handler;//控制函数处理器
	char name[V4L2_SUBDEV_NAME_SIZE];//子设备的名称
	u32 grp_id;//子设备所在的组标识
	void *dev_priv;//子设备私有数据指针，一般指向总线接口的客户端
	void *host_priv;//子设备私有的数据指针，一般指向总线接口的host端
	struct video_device *devnode;//设备节点
	struct device *dev;
